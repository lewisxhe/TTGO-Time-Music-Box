# 1 "D:/ESP32/esp-idf-v3.1-rc1/components/spiffs/spiffs/src/spiffs_hydrogen.c"
...
# 9 "D:/ESP32/esp-idf-v3.1-rc1/components/spiffs/spiffs/src/spiffs_hydrogen.c" 2
...
s32_t spiffs_object_index_consistency_check(
    spiffs *fs);
# 10 "D:/ESP32/esp-idf-v3.1-rc1/components/spiffs/spiffs/src/spiffs_hydrogen.c" 2


static s32_t spiffs_fflush_cache(spiffs *fs, spiffs_file fh);
# 26 "D:/ESP32/esp-idf-v3.1-rc1/components/spiffs/spiffs/src/spiffs_hydrogen.c"
u8_t SPIFFS_mounted(spiffs *fs) {
  return ((fs)->mounted != 0);
}
...
s32_t SPIFFS_format(spiffs *fs) {
  ...
  if (!(((fs))->config_magic == (0x20090315))) { (fs)->err_code = -10024; return -10024; };
  if (((fs)->mounted != 0)) {
    fs->err_code = -10026;
    return -1;
  }
  ...
  while (bix < fs->block_count) {
    ...
    if ((res) < 0) { (fs)->err_code = (res); spiffs_api_unlock(fs); return (res); };
    bix++;
  }

  spiffs_api_unlock(fs);
...
# 86 "D:/ESP32/esp-idf-v3.1-rc1/components/spiffs/spiffs/src/spiffs_hydrogen.c"
                                           ;
  void *user_data;
  spiffs_api_lock(fs);
  user_data = fs->user_data;
  memset(fs, 0, sizeof(spiffs));
  do{memcpy((&fs->cfg),(config),(sizeof(spiffs_config)));}while(0);
  ...
  fs->cache = cache;
  fs->cache_size = (cache_size > (((fs)->cfg.log_page_size)*32)) ? ((fs)->cfg.log_page_size)*32 : cache_size;
...
void SPIFFS_unmount(spiffs *fs) {
  ;
  if (!((fs)->config_magic == (0x20090315)) || !((fs)->mounted != 0)) return;
  spiffs_api_lock(fs);
  u32_t i;
...
s32_t SPIFFS_creat(spiffs *fs, const char *path, spiffs_mode mode) {
  ...
  (void)mode;
  if (!(((fs))->config_magic == (0x20090315))) { (fs)->err_code = -10024; return -10024; };
  if (!(((fs))->mounted != 0)) { (fs)->err_code = -10000; return -10000; };
  if (strlen(path) > (32) - 1) {
    if ((-10036) < 0) { (fs)->err_code = (-10036); return (-10036); };
  }
  spiffs_api_lock(fs);
  spiffs_obj_id obj_id;
  s32_t res;
  ...
  if ((res) < 0) { (fs)->err_code = (res); spiffs_api_unlock(fs); return (res); };
  res = spiffs_object_create(fs, obj_id, (const u8_t*)path, 0, (1), 0);
  if ((res) < 0) { (fs)->err_code = (res); spiffs_api_unlock(fs); return (res); };
  spiffs_api_unlock(fs);
  return 0;
...
spiffs_file SPIFFS_open(spiffs *fs, const char *path, spiffs_flags flags, spiffs_mode mode) {
  ...
  if ((flags & (1<<2)) && res == -10002) {
    ...
    if ((res) < 0) { (fs)->err_code = (res); spiffs_api_unlock(fs); return (res); };
    res = spiffs_object_create(fs, obj_id, (const u8_t*)path, 0, (1), &pix);
    if (res < 0) {
      spiffs_fd_return(fs, fd->file_nbr);
    }
    if ((res) < 0) { (fs)->err_code = (res); spiffs_api_unlock(fs); return (res); };
...
spiffs_file SPIFFS_open_by_page(spiffs *fs, spiffs_page_ix page_ix, spiffs_flags flags, spiffs_mode mode) {
  ...
  if ((((page_ix) % ( ((fs)->cfg.log_block_size)/((fs)->cfg.log_page_size) )) < (((1) > ((( ((fs)->cfg.log_block_size)/((fs)->cfg.log_page_size) ) * sizeof(spiffs_obj_id)) / ((fs)->cfg.log_page_size)) ? (1) : ((( ((fs)->cfg.log_block_size)/((fs)->cfg.log_page_size) ) * sizeof(spiffs_obj_id)) / ((fs)->cfg.log_page_size))) ))) {
    res = -10031;
    spiffs_fd_return(fs, fd->file_nbr);
    if ((res) < 0) { (fs)->err_code = (res); spiffs_api_unlock(fs); return (res); };
  }
...
static s32_t spiffs_hydro_read(spiffs *fs, spiffs_file fh, void *buf, s32_t len) {
  ...
  if (fd->fdoffset + len >= fd->size) {
    ...
    if (avail <= 0) {
      if ((-10003) < 0) { (fs)->err_code = (-10003); spiffs_api_unlock(fs); return (-10003); };
    }
...
static s32_t spiffs_hydro_write(spiffs *fs, spiffs_fd *fd, void *buf, u32_t offset, s32_t len) {
  ...
  s32_t res = 0;
  s32_t remaining = len;
  if (fd->size != (u32_t)(-1) && offset < fd->size) {
    s32_t m_len = (((s32_t)(fd->size - offset)) < (len) ? ((s32_t)(fd->size - offset)) : (len));
...
s32_t SPIFFS_write(spiffs *fs, spiffs_file fh, void *buf, s32_t len) {
  ...
  if ((fd->flags & (1<<0))) {
    fd->fdoffset = fd->size == (u32_t)(-1) ? 0 : fd->size;
  }
  offset = fd->fdoffset;
  ...
  if (fd->flags & (1<<0)) {
    ...
    if (fd->cache_page) {
      offset = ((offset) > (fd->cache_page->offset + fd->cache_page->size) ? (offset) : (fd->cache_page->offset + fd->cache_page->size));
    }
  ...
  if ((fd->flags & (1<<5)) == 0) {
    if (len < (s32_t)((fs)->cfg.log_page_size)) {

      u8_t alloc_cpage = 1;
      if (fd->cache_page) {

        if (offset < fd->cache_page->offset ||
            offset > fd->cache_page->offset + fd->cache_page->size ||
            offset + len > fd->cache_page->offset + ((fs)->cfg.log_page_size))
        {

         
                                                                                                         ;
          res = spiffs_hydro_write(fs, fd,
              ((u8_t *)(&((((spiffs_cache *)((fs)->cache)))->cpages[(fd->cache_page->ix) * (sizeof(spiffs_cache_page) + ((fs)->cfg.log_page_size))])) + sizeof(spiffs_cache_page)),
      ...
      if (fd->cache_page) {
        u32_t offset_in_cpage = offset - fd->cache_page->offset;
                                         ...
                                         ;
        spiffs_cache *cache = ((spiffs_cache *)((fs)->cache));
        u8_t *cpage_data = ((u8_t *)(&((cache)->cpages[(fd->cache_page->ix) * (sizeof(spiffs_cache_page) + ((fs)->cfg.log_page_size))])) + sizeof(spiffs_cache_page));
# 558 "D:/ESP32/esp-idf-v3.1-rc1/components/spiffs/spiffs/src/spiffs_hydrogen.c"
        do{memcpy((&cpage_data[offset_in_cpage]),(buf),(len));}while(0);
        fd->cache_page->size = ((fd->cache_page->size) > (offset_in_cpage + len) ? (fd->cache_page->size) : (offset_in_cpage + len));
        fd->fdoffset += len;
        spiffs_api_unlock(fs);
        return len;
      } else {
...
s32_t SPIFFS_lseek(spiffs *fs, spiffs_file fh, s32_t offs, int whence) {
  ...
  s32_t file_size = fd->size == (u32_t)(-1) ? 0 : fd->size;
  ...
  if (offs < 0) {
    if ((-10040) < 0) { (fs)->err_code = (-10040); spiffs_api_unlock(fs); return (-10040); };
  }
  if (offs > file_size) {
  ...
  spiffs_span_ix data_spix = (offs > 0 ? (offs-1) : 0) / ( ((fs)->cfg.log_page_size) - sizeof(spiffs_page_header) );
  spiffs_span_ix objix_spix = ((data_spix) < ((((fs)->cfg.log_page_size) - sizeof(spiffs_page_object_ix_header))/sizeof(spiffs_page_ix)) ? 0 : (1+((data_spix)-((((fs)->cfg.log_page_size) - sizeof(spiffs_page_object_ix_header))/sizeof(spiffs_page_ix)))/((((fs)->cfg.log_page_size) - sizeof(spiffs_page_object_ix))/sizeof(spiffs_page_ix))));
  if (fd->cursor_objix_spix != objix_spix) {
    spiffs_page_ix pix;
    res = spiffs_obj_lu_find_id_and_span(
        fs, fd->obj_id | ((spiffs_obj_id)(1<<(8*sizeof(spiffs_obj_id)-1))), objix_spix, 0, &pix);
    if ((res) < 0) { (fs)->err_code = (res); spiffs_api_unlock(fs); return (res); };
...
static s32_t spiffs_stat_pix(spiffs *fs, spiffs_page_ix pix, spiffs_file fh, spiffs_stat *s) {
  ...
  spiffs_obj_id obj_id;
  s32_t res =spiffs_phys_rd((fs), ((2<<0) | (5<<2)), (fh), (( ((fs)->cfg.phys_addr) + (pix) * ((fs)->cfg.log_page_size) )), (sizeof(spiffs_page_object_ix_header)), ((u8_t *)&objix_hdr))
                                                                                              ;
  if ((res) < 0) { (fs)->err_code = (res); return (res); };

  u32_t obj_id_addr = ( ((fs)->cfg.phys_addr) + (( (pix) / ( ((fs)->cfg.log_block_size)/((fs)->cfg.log_page_size) ) ))* ((fs)->cfg.log_block_size) ) +
      ( (pix) % ( ((fs)->cfg.log_block_size)/((fs)->cfg.log_page_size) ) - (((1) > ((( ((fs)->cfg.log_block_size)/((fs)->cfg.log_page_size) ) * sizeof(spiffs_obj_id)) / ((fs)->cfg.log_page_size)) ? (1) : ((( ((fs)->cfg.log_block_size)/((fs)->cfg.log_page_size) ) * sizeof(spiffs_obj_id)) / ((fs)->cfg.log_page_size))) ) ) * sizeof(spiffs_obj_id);
  res =spiffs_phys_rd((fs), ((0<<0) | (5<<2)), (fh), (obj_id_addr), (sizeof(spiffs_obj_id)), ((u8_t *)&obj_id))
                                                          ;
  if ((res) < 0) { (fs)->err_code = (res); return (res); };

  s->obj_id = obj_id & ~((spiffs_obj_id)(1<<(8*sizeof(spiffs_obj_id)-1)));
  s->type = objix_hdr.type;
  s->size = objix_hdr.size == (u32_t)(-1) ? 0 : objix_hdr.size;
  s->pix = pix;
  strncpy((char *)s->name, (char *)objix_hdr.name, (32));

  do{memcpy((s->meta),(objix_hdr.meta),((4)));}while(0);
...
s32_t SPIFFS_rename(spiffs *fs, const char *old_path, const char *new_path) {
  ...
  if (!(((fs))->config_magic == (0x20090315))) { (fs)->err_code = -10024; return -10024; };
  if (!(((fs))->mounted != 0)) { (fs)->err_code = -10000; return -10000; };
  if (strlen(new_path) > (32) - 1 ||
      strlen(old_path) > (32) - 1) {
    if ((-10036) < 0) { (fs)->err_code = (-10036); return (-10036); };
  }
  spiffs_api_lock(fs);
...
static s32_t spiffs_read_dir_v(
  ...
  res = spiffs_phys_rd((fs), ((1<<0) | (5<<2)), (0), (( ((fs)->cfg.phys_addr) + (pix) * ((fs)->cfg.log_page_size) )), (sizeof(spiffs_page_object_ix_header)), ((u8_t *)&objix_hdr))
                                                                                                 ;
  if (res != 0) return res;
  if ((obj_id & ((spiffs_obj_id)(1<<(8*sizeof(spiffs_obj_id)-1)))) &&
    ...
    e->size = objix_hdr.size == (u32_t)(-1) ? 0 : objix_hdr.size;
    e->pix = pix;

    do{memcpy((e->meta),(objix_hdr.meta),((4)));}while(0);
...
struct spiffs_dirent *SPIFFS_readdir(spiffs_DIR *d, struct spiffs_dirent *e) {
  ...
  if (res == 0) {
    d->block = bix;
    d->entry = entry + 1;
    e->obj_id &= ~((spiffs_obj_id)(1<<(8*sizeof(spiffs_obj_id)-1)));
    ret = e;
  } else {
    d->fs->err_code = res;
  }
  spiffs_api_unlock(d->fs);
  return ret;
}

s32_t SPIFFS_closedir(spiffs_DIR *d) {
  ;
  if (!(((d->fs))->config_magic == (0x20090315))) { (d->fs)->err_code = -10024; return -10024; };
  if (!(((d->fs))->mounted != 0)) { (d->fs)->err_code = -10000; return -10000; };
  return 0;
}
...
s32_t SPIFFS_info(spiffs *fs, u32_t *total, u32_t *used) {
  ...
  u32_t pages_per_block = ( ((fs)->cfg.log_block_size)/((fs)->cfg.log_page_size) );
  u32_t blocks = fs->block_count;
  u32_t obj_lu_pages = (((1) > ((( ((fs)->cfg.log_block_size)/((fs)->cfg.log_page_size) ) * sizeof(spiffs_obj_id)) / ((fs)->cfg.log_page_size)) ? (1) : ((( ((fs)->cfg.log_block_size)/((fs)->cfg.log_page_size) ) * sizeof(spiffs_obj_id)) / ((fs)->cfg.log_page_size))) );
  u32_t data_page_size = ( ((fs)->cfg.log_page_size) - sizeof(spiffs_page_header) );
  u32_t total_data_pages = (blocks - 2) * (pages_per_block - obj_lu_pages) + 1;
...
s32_t SPIFFS_eof(spiffs *fs, spiffs_file fh) {
  ...
  res = (fd->fdoffset >= (fd->size == (u32_t)(-1) ? 0 : fd->size));
...
s32_t SPIFFS_ix_map(spiffs *fs, spiffs_file fh, spiffs_ix_map *map,
  ...
  if (fd->ix_map) {
    if ((-10038) < 0) { (fs)->err_code = (-10038); spiffs_api_unlock(fs); return (-10038); };
  }

  map->map_buf = map_buf;
  map->offset = offset;

  map->start_spix = offset / ( ((fs)->cfg.log_page_size) - sizeof(spiffs_page_header) );
  map->end_spix = (offset + len) / ( ((fs)->cfg.log_page_size) - sizeof(spiffs_page_header) );
...
s32_t SPIFFS_ix_unmap(spiffs *fs, spiffs_file fh) {
  ...
  if (fd->ix_map == 0) {
    if ((-10037) < 0) { (fs)->err_code = (-10037); spiffs_api_unlock(fs); return (-10037); };
  }
...
s32_t SPIFFS_ix_remap(spiffs *fs, spiffs_file fh, u32_t offset) {
  ...
  s32_t spix_diff = offset / ( ((fs)->cfg.log_page_size) - sizeof(spiffs_page_header) ) - map->start_spix;
  map->offset = offset;
...
s32_t SPIFFS_bytes_to_ix_map_entries(spiffs *fs, u32_t bytes) {
  ...
  return (bytes + ( ((fs)->cfg.log_page_size) - sizeof(spiffs_page_header) ) ) / ( ((fs)->cfg.log_page_size) - sizeof(spiffs_page_header) );
}

s32_t SPIFFS_ix_map_entries_to_bytes(spiffs *fs, u32_t map_page_ix_entries) {
  if (!(((fs))->config_magic == (0x20090315))) { (fs)->err_code = -10024; return -10024; };
  return map_page_ix_entries * ( ((fs)->cfg.log_page_size) - sizeof(spiffs_page_header) );
}
