# 1 "D:/ESP32/esp-idf-v3.1-rc1/components/bt/bluedroid/external/sbc/encoder/srce/sbc_encoder.c"
# 1 "D:\\ESP32\\esp-idf-v3.1-rc1\\ESP32_PIC_A2DP\\build\\bt//"
# 1 "<built-in>"
# 1 "<command-line>"
# 1 "D:/ESP32/esp-idf-v3.1-rc1/components/bt/bluedroid/external/sbc/encoder/srce/sbc_encoder.c"
# 25 "D:/ESP32/esp-idf-v3.1-rc1/components/bt/bluedroid/external/sbc/encoder/srce/sbc_encoder.c"
...
# 26 "D:/ESP32/esp-idf-v3.1-rc1/components/bt/bluedroid/external/sbc/encoder/srce/sbc_encoder.c" 2
...
# 27 "D:/ESP32/esp-idf-v3.1-rc1/components/bt/bluedroid/external/sbc/encoder/srce/sbc_encoder.c" 2
...
# 28 "D:/ESP32/esp-idf-v3.1-rc1/components/bt/bluedroid/external/sbc/encoder/srce/sbc_encoder.c" 2
...
# 29 "D:/ESP32/esp-idf-v3.1-rc1/components/bt/bluedroid/external/sbc/encoder/srce/sbc_encoder.c" 2



SINT16 EncMaxShiftCounter;
# 88 "D:/ESP32/esp-idf-v3.1-rc1/components/bt/bluedroid/external/sbc/encoder/srce/sbc_encoder.c"
typedef struct {
    UINT8 use;
    UINT8 idx;
} tSBC_FR_CB;
...
# 113 "D:/ESP32/esp-idf-v3.1-rc1/components/bt/bluedroid/external/sbc/encoder/srce/sbc_encoder.c"
SINT32 s32LRDiff[16] = {0};
SINT32 s32LRSum[16] = {0};
...
void SBC_Encoder(SBC_ENC_PARAMS *pstrEncParams)
{
    ...
    do {
        ...
        pstrEncParams->ps16NextPcmBuffer += s32Ch * s32NumOfBlocks;

        for (s32Sb = 0; s32Sb < s32Ch; s32Sb++) {
            SbBuffer = pstrEncParams->s32SbBuffer + s32Sb;
            s32MaxValue = 0;
            for (s32Blk = s32NumOfBlocks; s32Blk > 0; s32Blk--) {
                if (s32MaxValue < ( (*SbBuffer >= 0) ? *SbBuffer : (-*SbBuffer) )) {
                    s32MaxValue = ( (*SbBuffer >= 0) ? *SbBuffer : (-*SbBuffer) );
                }
                SbBuffer += s32Ch;
            }
            ...
            for (s32Sb = 0; s32Sb < s32NumOfSubBands - 1; s32Sb++) {
                ...
                for (s32Blk = 0; s32Blk < s32NumOfBlocks; s32Blk++) {
                    *pSum = (*SbBuffer + * (SbBuffer + s32NumOfSubBands)) >> 1;
                    if (( (*pSum >= 0) ? *pSum : (-*pSum) ) > s32MaxValue) {
                        s32MaxValue = ( (*pSum >= 0) ? *pSum : (-*pSum) );
                    }
                    pSum++;
                    *pDiff = (*SbBuffer - * (SbBuffer + s32NumOfSubBands)) >> 1;
                    if (( (*pDiff >= 0) ? *pDiff : (-*pDiff) ) > s32MaxValue2) {
                        s32MaxValue2 = ( (*pDiff >= 0) ? *pDiff : (-*pDiff) );
                    }
                    pDiff++;
                    SbBuffer += s32Ch;
                }
        ...
        if ((pstrEncParams->s16ChannelMode == 2) || (pstrEncParams->s16ChannelMode == 3)) {
        ...
        {if(sbc_prtc_cb.init == 0){sbc_prtc_cb.init=1; pu8[0] &= ~0x10;}};
        {{p_last=&sbc_prtc_cb.fr[1]; p_cur=&sbc_prtc_cb.fr[0]; p_last->idx = p_cur->idx; p_last->use = p_cur->use;};{p_cur->use = pu8[3] & 0x64; p_cur->idx = (((pu8[3]) & 0x3) + (((pu8[3]) & 0x30) >> 2));};sbc_prtc_cb.index = (p_cur->use)?0:1;};
        ...
        {idx = sbc_prtc_cb.fr[sbc_prtc_cb.index].idx; if(idx > 0){if((idx&1)&&(pstrEncParams->u16PacketLength > (sbc_prtc_cb.base+(idx<<1)))) {tmp2=idx<<1; tmp=(&pu8[sbc_prtc_cb.base])[idx];(&pu8[sbc_prtc_cb.base])[idx]=(&pu8[sbc_prtc_cb.base])[tmp2];(&pu8[sbc_prtc_cb.base])[tmp2]=tmp;} else{tmp2=(&pu8[sbc_prtc_cb.base])[idx]; tmp=(tmp2>>5)+(tmp2<<3);(&pu8[sbc_prtc_cb.base])[idx]=(UINT8)tmp;}}};
...
void SBC_Encoder_Init(SBC_ENC_PARAMS *pstrEncParams)
{
    ...
    if (pstrEncParams->s16ChannelMode == 0) {
    ...
    HeaderParams |= ((pstrEncParams->s16NumOfSubBands >> 3) & 1);
    pstrEncParams->FrameHeader = HeaderParams;

    if (pstrEncParams->s16NumOfSubBands == 4) {
        if (pstrEncParams->s16NumOfChannels == 1) {
            EncMaxShiftCounter = ((((8*10*2) + 64) - 4 * 10) >> 2) << 2;
        } else {
            EncMaxShiftCounter = ((((8*10*2) + 64) - 4 * 10 * 2) >> 3) << 2;
        }
    } else {
        if (pstrEncParams->s16NumOfChannels == 1) {
            EncMaxShiftCounter = ((((8*10*2) + 64) - 8 * 10) >> 3) << 3;
        } else {
            EncMaxShiftCounter = ((((8*10*2) + 64) - 8 * 10 * 2) >> 4) << 3;
        }
    }

    {if (appl_trace_level >= 4 && (((2) > (3) ? (2) : (3)) >= 4)) {esp_log_write(ESP_LOG_DEBUG, "BT_APPL", "D" " (%d) %s: " "SBC_Encoder_Init : bitrate %d, bitpool %d" "\033[0m" "\n", esp_log_timestamp(), "BT_APPL", pstrEncParams->u16BitRate, pstrEncParams->s16BitPool); };}
                                                                          ;
