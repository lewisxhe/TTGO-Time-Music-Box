# 1 "D:/ESP32/esp-idf-v3.1-rc1/components/coap/libcoap/src/block.c"
# 1 "D:\\ESP32\\esp-idf-v3.1-rc1\\ESP32_PIC_A2DP\\build\\coap//"
# 1 "<built-in>"
# 1 "<command-line>"
# 1 "D:/ESP32/esp-idf-v3.1-rc1/components/coap/libcoap/src/block.c"
# 9 "D:/ESP32/esp-idf-v3.1-rc1/components/coap/libcoap/src/block.c"
...
# 10 "D:/ESP32/esp-idf-v3.1-rc1/components/coap/libcoap/src/block.c" 2
...
# 16 "D:/ESP32/esp-idf-v3.1-rc1/components/coap/libcoap/src/block.c" 2
...
# 17 "D:/ESP32/esp-idf-v3.1-rc1/components/coap/libcoap/src/block.c" 2
# 25 "D:/ESP32/esp-idf-v3.1-rc1/components/coap/libcoap/src/block.c"
unsigned int
coap_opt_block_num(const coap_opt_t *block_opt) {
  ...
  if (len > 1) {
    num = coap_decode_var_bytes(coap_opt_value((coap_opt_t *)block_opt),
    coap_opt_length(block_opt) - 1);
  }

  return (num << 4) | ((*(coap_opt_length(block_opt) ? (coap_opt_value((coap_opt_t *)block_opt) + (coap_opt_length(block_opt)-1)) : 0) & 0xF0) >> 4);
}
...
coap_get_block(coap_pdu_t *pdu, unsigned short type, coap_block_t *block) {
  ...
  ((block) ? (void)0 : __assert_func ("D:/ESP32/esp-idf-v3.1-rc1/components/coap/libcoap/src/block.c", 49, __func__, "block"));
  memset(block, 0, sizeof(coap_block_t));

  if (pdu && (option = coap_check_option(pdu, type, &opt_iter))) {
    unsigned int num;

    block->szx = (coap_opt_length(option) ? (*(coap_opt_length(option) ? (coap_opt_value((coap_opt_t *)option) + (coap_opt_length(option)-1)) : 0) & 0x07) : 0);
    if ((coap_opt_length(option) ? (*(coap_opt_length(option) ? (coap_opt_value((coap_opt_t *)option) + (coap_opt_length(option)-1)) : 0) & 0x08) : 0))
      block->m = 1;
...
coap_write_block_opt(coap_block_t *block, unsigned short type,
  ...
  ((pdu) ? (void)0 : __assert_func ("D:/ESP32/esp-idf-v3.1-rc1/components/coap/libcoap/src/block.c", 78, __func__, "pdu"));

  start = block->num << (block->szx + 4);
  if (data_length <= start) {
    coap_log_impl(LOG_DEBUG, "illegal block requested\n");
    return -2;
  }
  ...
  if (want <= avail) {
    ...
    if (data_length - start <= avail) {
      ...
      if (avail < 16) {
 coap_log_impl(LOG_DEBUG, "not enough space, even the smallest block does not fit");
 return -3;
      }
      coap_log_impl(LOG_DEBUG, "decrease block size for %zu to %d\n", avail, coap_fls(avail) - 5);
      szx = block->szx;
...
coap_add_block(coap_pdu_t *pdu, unsigned int len, const unsigned char *data,
  ...
  return coap_add_data(pdu,
         ((len - start) < ((unsigned int)(1 << (block_szx + 4))) ? (len - start) : ((unsigned int)(1 << (block_szx + 4)))),
         data + start);
}
